...
#include "pruning.h"
#include "solve.h"
#include "solve_multiple.h"
#include "utils.h"

#define max_moves 20

move_t *solve_continuous(coord_cube_t *original_cube) {
    ...
    move_t *phase1_solution = solve_phase1_continuous(cube);

    if (phase1_solution == NULL) {
        printf("failed to solve cube\n");
        free(solution);
        return NULL;
    } else {
        ...
        do {
            ...
            */
            ...
            if (is_phase1_solved(cube_stack[pivot])) {
                ...
                /*move_t *phase2_solution = solve_phase2_continuous(phase2_cube, pivot, 0.25);*/
                move_t *phase2_solution = solve_phase2_continuous(phase2_cube, pivot, 1.0);
                /*move_t *phase2_solution = solve_phase2_continuous(phase2_cube, pivot, 2.0);*/
                ...
                if (phase2_solution == NULL) {
                    ...
                    for (int i = 0; phase2_solution[i] != MOVE_NULL; i++) {
                        ...
                        solution[pivot + i] = phase2_solution[i];
                    }
                    solution[pivot + phase2_move_count] = MOVE_NULL;
                    printf("\n");
    ...
    for (int allowed_depth = 1; allowed_depth <= max_depth - starting_depth; allowed_depth++) {
        ...
        do {
            ...
            */
            ...
            long  end_time        = get_microseconds();
            float elapsed_seconds = (float)(end_time - start_time) / 1000000.0;

            if (elapsed_seconds > timeout) {
                printf("phase2 search timeout out\n");
                free(solution);
                solution = NULL;
                goto solution_found;
            }
